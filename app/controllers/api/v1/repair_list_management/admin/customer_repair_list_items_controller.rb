class Api::V1::RepairListManagement::Admin::CustomerRepairListItemsController < Api::V1::BaseController
    before_action :doorkeeper_authorize!
	before_action :validate_token!
    before_action :set_repair_list, only: [:index, :export]
    before_action :set_repair_list_item, only: [:show, :delete, :update]
    ...
    def upload
        begin
            CustomerRepairListItem.import(@repair_list_item_upload, params[:customer_repair_list_id])
    ...
    def export
        repair_list_items = CustomerRepairListItem.where(customer_repair_list: @repair_list)
        respond_to do |format|
            format.csv { send_data repair_list_items.export, filename: "Customer_Repair_List_Items_#{@repair_list.id}_#{Date.today}.csv", \
    ...
    def set_repair_list
        @repair_list = CustomerRepairList.find(params[:customer_repair_list_id])
    end

    def set_repair_list_item
        @repair_list_item = CustomerRepairListItem.find(params[:id])
    end

    def repair_list_item_params
        params.require(:customer_repair_list_item).permit(:container_repair_area_id, :uid, :container_damaged_area_id, :repair_type_id, \
            ...
            :location, :event_id, :length_id, :width_id, :unit_id, :is_mearsk_not_applicable, \
    ...
    def filter_items
        @repair_list_items = CustomerRepairListItem.where(customer_repair_list: @repair_list, deleted_at: nil)
