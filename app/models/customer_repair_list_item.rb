    ...
    def self.import(repair_list_item_upload, repair_list_id)
        success = ActiveRecord::Base.transaction do
            ...
            (2..spreadsheet.last_row).each do |i|
                ...
                if row['uid'].present? && CustomerRepairListItem.find_by(uid: row['uid'], customer_repair_list_id: repair_list_id).present?
                    repair_list_item = CustomerRepairListItem.find_by(uid: row['uid'], customer_repair_list_id: repair_list_id)
                else
                    repair_list_item = CustomerRepairListItem.new(customer_repair_list_id: repair_list_id, uid: row['uid'] )
                end
